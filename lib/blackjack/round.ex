defmodule Blackjack.Round do
  ...
  @doc false
  def start(players_ids, deck) do
    %Round{
      deck: deck,
      current_hand: nil,
      current_player_id: nil,
      all_players: players_ids,
      remaining_players: players_ids,
      successful_hands: [],
      instructions: []
    }
    |> start_new_hand()
  ...
  defp deal(round) do
    {:ok, card, deck} =
      with {:error, :empty} <- Blackjack.Deck.take(round.deck),
           do: Blackjack.Deck.take(Blackjack.Deck.shuffled())
  ...
  defp instructions_and_state(round) do
    round |> tell_current_player_to_move() |> take_instructions()
  end
