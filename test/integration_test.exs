defmodule Blackjack.IntegrationTest do
  ...
  def deal_card(test_pid, player_id, card), do: send(test_pid, {player_id, {:deal_card, card}})

  def move(test_pid, player_id), do: send(test_pid, {player_id, :move})

  def busted(test_pid, player_id), do: send(test_pid, {player_id, :busted})

  def unauthorized_move(test_pid, player_id), do: send(test_pid, {player_id, :unauthorized_move})

  def winners(test_pid, player_id, winner_ids),
    do: send(test_pid, {player_id, {:winners, winner_ids}})
end
